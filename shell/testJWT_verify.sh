...
PHASE1=../circuits/ptau/powersOfTau28_hez_final_25.ptau
BUILD_DIR=../build/testJWT_verify
CIRCUIT_NAME=testJWT_verify
...
start=`date +%s`
circom ../circuits/"$CIRCUIT_NAME".circom --r1cs --wasm --sym --O0 --output "$BUILD_DIR"
end=`date +%s`
echo "DONE ($((end-start))s)"

echo "****GENERATING WITNESS FOR SAMPLE INPUT****"
start=`date +%s`
node "$BUILD_DIR"/"$CIRCUIT_NAME"_js/generate_witness.js "$BUILD_DIR"/"$CIRCUIT_NAME"_js/"$CIRCUIT_NAME".wasm testJWT_verify.json "$BUILD_DIR"/witness.wtns
end=`date +%s`
echo "DONE ($((end-start))s)"
...
snarkjs zkey export solidityverifier "$BUILD_DIR"/"$CIRCUIT_NAME".zkey "$BUILD_DIR"/DID_Verifier.sol
